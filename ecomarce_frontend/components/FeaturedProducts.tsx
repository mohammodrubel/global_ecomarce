...
import { useGetAllfetchersProductsQuery } from "@/redux/fetchers/products/productsApi";
...
function FeaturedProducts() {
  const { data: products, isLoading } =
    useGetAllfetchersProductsQuery(undefined);
  ...
  // Fetch only active Fetchers advertisement
  const advertisementItem = advertisements?.data?.find(
    (ad: any) => ad.active && ad.action === "Fetchers"
  );
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 sm:mb-12 gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold mb-2">
              {advertisementItem?.title || "Featured Products"}
            </h2>
            <p className="text-gray-600 text-sm sm:text-base">
              {advertisementItem
                ? "Specially picked advertisement products ✨"
